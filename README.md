...
* **Storage solutions in AKS**: If a managed database doesn't meet your application needs, AKS has multiple storage options available to store consistent data:
  ...
  * **Blob**: Azure blob storage is Microsoft's object storage platform. It is accessible via an HTTP API or through the SDKs.
    * Evaluate which [data redundancy](https://docs.microsoft.com/en-us/azure/storage/common/storage-redundancy?msclkid=fe4292eab60111ecaf0ba3d405479361) (defined at storage account level) is required and which [performance tier](https://docs.microsoft.com/en-us/azure/storage/blobs/access-tiers-overview) of blob storage is needed.
    * Consider which [authentication method](https://docs.microsoft.com/en-us/azure/storage/common/authorize-data-access?msclkid=7533d8c9b60211eca7a1e7cce8329b76) to blob you want to use: storage key, SAS or AAD-integrated.
  ...
  * **Other**: There are a number of specialized storage solutions in Azure that can also be integrated with Kubernetes. These won't be explored in depth in this document.
    * [HPC cache](https://azure.microsoft.com/en-us/services/hpc-cache/): A high-performance cache that is optimized for high-performance-compute workloads or graphical rendering.  
    * [ADLS gen 2](https://docs.microsoft.com/en-us/azure/storage/blobs/data-lake-storage-introduction?msclkid=c6c65302b60211eca7063954baa75849): A special type of Azure blob storage optimized for big data workloads like Hadoop and Spark.
...
* For Azure Files:
  ...
  * Consider carefully whether you want Kubernetes to create the file shares or if you want to create them statically yourself.
* For Azure Blob:
